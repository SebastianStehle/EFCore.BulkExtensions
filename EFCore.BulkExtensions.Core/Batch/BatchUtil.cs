...
public static class BatchUtil
{
    ...
    public static (string, List<DbParameter>) PrepareSqlUpdate(DbContext context, Type type, List<DbParameter> sqlParameters, string sql, string sqlSET, string tableAlias, string tableAliasSufixAs, string topStatement, string leadingComments, StringBuilder? sqlColumns)
    {
        ...
        if (databaseType == SqlType.PostgreSql)
        {
            ...
            {
                dynamic npgsqlParam = SqlAdaptersMapping.DbServer(context).QueryBuilder.CreateParameter(param.ParameterName, param.Value);
        ...
        else if (databaseType == SqlType.MySql)
        {
            ...
            foreach (var param in sqlParameters)
            {
                dynamic mysqlParam = SqlAdaptersMapping.DbServer(context).QueryBuilder.CreateParameter(param.ParameterName, param.Value);
                ...
                {
                    var dbtypeJsonb = SqlAdaptersMapping.DbServer(context).QueryBuilder.Dbtype();
                    SqlAdaptersMapping.DbServer(context).QueryBuilder.SetDbTypeParam(mysqlParam, dbtypeJsonb);
                }
